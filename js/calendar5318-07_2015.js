...
(function($) {
    ...
    $.fn.calendar5318 = function() {
        ...
        // Instantiation of the calendar
        var calendar = new Calendar();


        // Auxiliary variables
        var inputBox = null;   // BoundingClientRect
        var inputWidth = 0;
        var inputHeight = 0;

        var $clnImage = null;   // the calendar image (icon)
        var $clnDiv = null;       // the calendar DIV


        // Loop of elements selected to add a calendar
        this.each( function() {
        ...
        // Build a calendar wrapper DIV's
            $inputElement.wrap( '<div class="calendar-box"></div>' );
            $inputElement.wrap( '<div class="calendar-img-box"></div>' );
            $inputElement.parent('.calendar-img-box').append( '<img src="img/calendar.png" alt="Calendar icon" title="Click to open the calendar" class="calendar-img">' );
            $inputElement.parents('.calendar-box').append( '<div class="calendar"></div>' );

        // Define jQuery's objects for the calendar image and the calendar main DIV
            $clnImage = $inputElement.next('.calendar-img');
            $clnDiv = $inputElement.parents('.calendar-box').children('.calendar');

        // The INPUT box sizes
            inputBox = $inputElement.get(0).getBoundingClientRect();
            inputWidth = Math.round( inputBox.right - inputBox.left );
            inputHeight = Math.round( inputBox.bottom - inputBox.top );

        // Recalculate height of the calendar image (icon) that should be placed inside the INPUT
            $clnImage.css( { 'height': (inputHeight - 2) + 'px' } );
        // Calculate the top calendar position on a basis of the INPUT height
            $clnDiv.css( { 'top': (inputHeight + 1) + 'px' } );

        // Recalculate width of the calculator container or the input width
        // --- if we do NOT recalc this width, the input is responsive and flexible rather than fixed
            // if ( inputWidth != 0   &&   inputHeight != 0 ) {
                // $inputElement.parent('.calendar-img-box').css( { 'width': inputWidth + 'px' } );
//              $inputElement.css( { 'width': (inputWidth - 32) + 'px' } );
            // }
//          else if {
...
//          }
        ...
        //     Calendar events
        // ==============================================
            (function( inputElement, clnImage, clnDiv ) {
                // Open a calendar by a click on the calendar image (icon)
                clnImage.on('click', function(event) {
                    calendar.show( clnDiv );
                    event.stopPropagation();
                });

                // Date picking out from the calendar DIV with the class="calendar"
                clnDiv.on('click', 'td', function(event) {
                    var tdPicked = $(this);
                    inputElement.val( calendar.pickDate( tdPicked ) );
                    calendar.closeCalendar();
                });

                // Recalculation after the month or year is changed. Controls that allow such changes have the class="change-calendar"
                clnDiv.on('click', '.change-calendar', function(event) {
                    var clickedButton = $(this);
                    calendar.changeMonthYear( clickedButton, clnDiv );
                });

                // This allows avoiding the closing of the calendar when clicking within it
                clnDiv.on('click', function(event) {
                    event.stopPropagation();
                });

            }( $inputElement, $clnImage, $clnDiv ));
...
// =======================================================================
//     Calendar constructor and methods
// =======================================================================
...
//                        pickedDay = '' + addZero(day) + '-' + addZero(month + 1) + '-' + year;     // Format of Date: now it is set to   DD-MM-YYYY
                        pickedDay = '' + year + '-' + addZero(month + 1) + '-' + addZero(day);     // Format of Date: now it is set to   YYYY-MM-DD
                        dayText = '' + day;
                        day++;
                    }
